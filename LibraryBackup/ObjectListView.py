...
# ----------------------------------------------------------------------------
# Change log:
# 2019-02-28  PCT   - SetEmptyListMsgColors() - Added.  fg & bg colouring options for stEmptyListMsg
# 2018-03-29  PCT   - SetSortColumn() - Corrected. Sort indicator not clearing when sort was reset to default
# 2018-03-29  PCT   - _HandleTypingEvent() - Corrected.  TypeError when keydown filtering
...
class ObjectListView(wx.ListCtrl):
    ...
    * useAlternateBackColors
    ...
    def SetEmptyListMsgColors(self, fg=wx.LIGHT_GREY, bg=wx.WHITE):
        """
        What colors should the empty list msg be rendered?
        """
        self.stEmptyListMsg.SetForegroundColour(fg)
        self.stEmptyListMsg.SetBackgroundColour(bg)
    ...
    def SetSortColumn(self, column, resortNow=False):
        ...
        """
        if column is None:
            oldSortColumnIndex = self.sortColumnIndex  # PCT added this line to fix search indicators not clearing
            self.sortColumnIndex = -1
        ...
        if resortNow:
            ...
            # self._UpdateColumnSortIndicators()  # PCT modified this line to fix search indicators not clearing
    ...
    def _HandleTypingEvent(self, evt):
        ...
        if evt.GetUnicodeKey() == 0:
            ...
            # uniChar = evt.GetUnicodeKey()     # PCT TypeError: 'in <string>' requires string as left operand, not int
